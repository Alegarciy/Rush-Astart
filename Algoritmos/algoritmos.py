...
def aEstrella(tablero: Tablero, profundidadMaxima: int= 1000) -> Resultado:
  profundidad=0
  ...
  while len(listaOrdenada) & profundidad < profundidadMaxima:
    ...
    for movimientoPosible in nodoActual.tablero.getMovimientos():
      tableroHijo = nodoActual.tablero.moverVehiculo(movimiento=movimientoPosible)
      ...
      if tableroHijo not in nodosVisitados:
        ...
        nodo = NodoHijo(
          ...
          valor=tableroHijo.MinimoDePasosParaDespejarPaso + profundidad
        )
